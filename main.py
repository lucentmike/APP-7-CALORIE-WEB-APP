...
class Results(MethodView):
    def post(self):
        tempform = CalorieForm(request.form)

        city = tempform.city.data

        country = tempform.country.data

        temperature = Temp(country, city).get()

        calories = Calorie(float(tempform.weight.data), float(tempform.height.data), float(tempform.age.data), temperature)

        calorie_form = CalorieForm()

        return render_template('calories_form_page.html', caloriesform=calorie_form, result = True, calories = calories.calculate())
...
app.add_url_rule('/calories_form', view_func=Results.as_view('calories_form'))
